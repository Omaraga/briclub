...
class BedsController extends Controller
{
    ...
    public function actionDeleteMtrix()
    {
        if(!User::isAccess('superadmin')){
            Yii::$app->user->logout();
            return $this->goHome();
    ...
    public function actionNew()
    {
        ...
        /*MatrixRef::plusToRefMatrix(16396,1,false,1);
		MatrixRef::plusToRefMatrix(16396,1,false,1);
		MatrixRef::plusToRefMatrix(16396,1,false,1);
		MatrixRef::plusToRefMatrix(16396,1,false,1);

		MatrixRef::plusToRefMatrix(16396,2,false,1);
		MatrixRef::plusToRefMatrix(16396,2,false,1);
		MatrixRef::plusToRefMatrix(16396,2,false,1);
		MatrixRef::plusToRefMatrix(16396,2,false,1);
		exit;

		/*$balans = \common\models\Actions::find()->where(['type'=>3])->sum('sum');
        $balans2 = \common\models\Actions::find()->where(['type'=>3])->andWhere(['>','time',1614535200])->andWhere(['<','time',1619805600])->sum('sum');
        echo $balans;
        echo "<br>";
        echo $balans2;
        echo "<br>";
        /*echo "<br>";
        echo $time= strtotime('01.03.2021');
        echo "<br>";
        echo date('d.m.Y H:i',$time);
        echo "<br>";
        echo $time= strtotime('31.04.2021');
        echo "<br>";
        echo date('d.m.Y H:i',$time);*/
        exit;
        /*$refs = Referals::find()->where(['parent_id'=>12811])->all();
        foreach ($refs as $ref) {
            $ref_user = User::findOne($ref['user_id']);
            if(!empty($ref_user)){
                if($ref_user['time_personal']>1616090400){
        ...
        exit;*/
        /*$mats = MatrixRef::find()->where(['platform_id'=>3,'reinvest'=>0])->all();
        foreach ($mats as $mat) {
            $mat2 = MatrixRef::find()->where(['user_id'=>$mat['user_id'],'platform_id'=>2,'reinvest'=>0])->one();
            if(empty($mat2)){
                echo User::findOne($mat['user_id'])['username'];
                echo "<br>";
            }
        }
        echo count($mats);
        ...
        /*$users = User::find()->where(['>','id',21842])->andWhere(['activ'=>1])->all();
        ...
        foreach ($users as $user) {
            ...
            echo $user['phone'];
            echo " - ";
            echo date('d.m.Y H:i',$user['time_personal']);
            echo "<br>";
        }*/

       /*MatrixRef::plusToRefMatrix(12750,3,false,1);
       exit;*/


        /*$start = strtotime('2021-02-17');
        echo $start;
        echo "<br>";
        echo date('d.m.Y H:i',$start);
        echo "<br>";
        $end = strtotime('2021-03-21');
        echo $end;
        echo "<br>";
        echo date('d.m.Y H:i',$end);
        echo "<br>";
        $mats = MatrixRef::find()->where(['platform_id'=>2,'slots'=>4])->all();
        $i = 0;
        foreach ($mats as $mat) {
            if ($mat['time']>$start and $mat['time'] <$end){
                $old = null;
                $old_mat = MatrixRef::find()->where(['user_id'=>$mat['user_id'],'platform_id'=>2])->all();
                foreach ($old_mat as $item) {
                    if($item['time']<$start){
                        $old = true;
                    }
                }
                if(!$old){
                    ...
                    echo $user['username']." - ".$user['fio']." - ".$user['phone']." - ".$user['email'];
        ...
        //$first_time = strtotime();
        /*$actions = Actions::find()->where(['type'=>3])->all();
        $all_sum = 0;
        ...
        echo "<br> 4 - стол: ";
        echo $mats1 = MatrixRef::find()->where(['platform_id'=>4])->count();
        echo "<br> 5 - стол: ";
        echo $mats2 = MatrixRef::find()->where(['platform_id'=>5])->count();
        echo "<br> 6 - стол: ";
        echo $mats3 = MatrixRef::find()->where(['platform_id'=>6])->count();
        exit;*/



        /*$tickets = EventTickets::find()->all();
        $i = 0;
        foreach ($tickets as $ticket) {
            $mat = MatrixRef::find()->where(['user_id'=>$ticket['user_id'],'platform_id'=>5])->one();
            if(!empty($mat)){
                $i++;
                $user = User::findOne($mat['user_id']);
                echo $user['username'];
                ...
                echo $user['fio'];
                echo " - ";
                echo EventTicketTypes::findOne($ticket['type_id'])['title'];
                echo "<br>";
            }
        }
        echo "Всего - ".$i;
        ...
        /*$mats = MatrixRef::find()->where(['platform_id'=>6])->all();
        $users = [];
        foreach ($mats as $mat) {
        ...
        $actions = Actions::find()->select('user_id')->where(['type'=>3])->distinct('user_id')->all();
        echo "<pre>";
        var_dump($actions);
        exit;
        ...
        $i = 0;
        foreach ($users as $user) {
            $children = Referals::find()->where(['parent_id'=>$user['id']])->count();
            if($children>99){
                echo $user['username']." - ".$children;
                echo "<br>";
                $i++;
            }
        }
        ...
        if($com->balans_pl0 == 0){
            MatrixRef::plusToRefMatrix2(12750,3,false,1,false,null,null,690);
        }
        $com->balans_pl0 = 1;
        $com->save();*/


        /*$user = User::findOne(['username'=>'Lider1']);
        $mats = MatrixRef::find()->where(['user_id'=>$user['id'],'deleted'=>2])->orderBy('platform_id asc')->all();
        foreach ($mats as $mat) {
            echo $mat['id']." Platform:".$mat['platform_id']." Children:".$mat['children']." Slots:".$mat['slots']." <br>";
        }
        exit;*/
        ...
        foreach ($users as $user) {
            ...
            $mats3 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>3])->andWhere(['>','slots',0])->count();
            if(empty($mats3)) continue;

            $mats6 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>6])->andWhere(['>','slots',0])->count();
            $clons1 = $clons1 + ($mats6*3);
            $clons2 = $clons2 + ($mats6*3);
            $clons3 = $clons3 + ($mats6*3);
            $clons4 = $clons4 + ($mats6*3);

            $mats5 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>5])->andWhere(['>','slots',0])->count();
            $clons1 = $clons1 + ($mats5*3);
            $clons2 = $clons2 + ($mats5*3);
            $clons3 = $clons3 + ($mats5*3);

            $mats4 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>4])->andWhere(['>','slots',0])->count();

            $clons1 = $clons1 + ($mats4*4);
            $clons2 = $clons2 + ($mats4*4);


            $clons1 = $clons1 + ($mats3*3);


            $closed2 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>1,'reinvest'=>1,'slots'=>4])->count();
            $closed3 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>2,'reinvest'=>1,'slots'=>4])->count();
            $closed4 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>3,'reinvest'=>1,'slots'=>4])->count();
            $clons2 += $closed2;
            $clons3 += $closed3;
            $clons4 += $closed4;


            $mat_clons1 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>1,'reinvest'=>1])->count();
            $mat_clons2 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>2,'reinvest'=>1])->count();
            $mat_clons3 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>3,'reinvest'=>1])->count();
            $mat_clons4 = MatrixRef::find()->where(['user_id'=>$user['id'],'platform_id'=>4,'reinvest'=>1])->count();

            if($clons1 != $mat_clons1){
                echo "Разница клонов 1 стол: ".($mat_clons1 - $clons1);
                echo "<br>";
                $print = 1;
            }
            if($clons2 != $mat_clons2){
                echo "Разница клонов 2 стол: ".($mat_clons2 - $clons2);
                echo "<br>";
                $print = 1;
            }
            if($clons3 != $mat_clons3){
                echo "Разница клонов 3 стол: ".($mat_clons3 - $clons3);
                echo "<br>";
                $print = 1;
            }
            if($clons4 != $mat_clons4){
                echo "Разница клонов 4 стол: ".($mat_clons4 - $clons4);
                echo "<br>";
                $print = 1;
            }
            if($print){
        ...
        /*echo date('d.m.Y H:i',1603042017);
        echo "<br>";
        //$actions = Actions::find()->all();e
        echo "1 - стол";
        echo MatrixRef::find()->where(['platform_id'=>1])->count();
        echo "<br>";
        echo "2 - стол";
        echo MatrixRef::find()->where(['platform_id'=>2])->count();
        echo "<br>";
        echo "3 - стол";
        echo MatrixRef::find()->where(['platform_id'=>3])->count();
        echo "<br>";
        echo "4 - стол";
        echo MatrixRef::find()->where(['platform_id'=>4])->count();
        echo "<br>";
        echo "5 - стол";
        echo MatrixRef::find()->where(['platform_id'=>5])->count();
        echo "<br>";
        echo "6 - стол";
        echo MatrixRef::find()->where(['platform_id'=>6])->count();
        echo "<br>";
        $all_sum = 0;
        $all_trs = 0;*/
        /*foreach ($actions as $action) {
       ...
       /* echo $all_trs;
        ...
        //MatrixRef::plusToRefMatrix(12801,3,false,1);
