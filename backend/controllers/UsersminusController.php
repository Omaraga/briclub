 ...
 */
class UsersminusController extends Controller
{
    /**
     * {@inheritdoc}
     */
     ...
     * @return mixed
     */
    public function actionIndex($username = null,$email=null,$phone=null,$fio=null,$platform=null,$from=null,$to=null,$newmatrix=null)
    {
        ...
        $dataProvider = new ActiveDataProvider([
            'query' => User::find()->where(['>','overdraft',0]),
            'sort' => [
                'defaultOrder' => [
                    'id' => SORT_DESC,
                ]
        ...
        if(!empty($platform)){
            if($platform == 11){
                $dataProvider = new ActiveDataProvider([
                    'query' => User::find()->where(['<','platform_id',1])->orWhere(['platform_id'=>null]),
                ]);
            }else{
                $dataProvider = new ActiveDataProvider([
                    'query' => User::find()->where(['platform_id'=>$platform]),
                ]);
            }
