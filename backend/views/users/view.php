...
$actions = \common\models\Actions::find()->where(['user_id'=>$model->id])->asArray()->orderBy('id desc')->all();
$withdraws = \common\models\Withdraws::find()->where(['user_id'=>$model->id])->asArray()->orderBy('id desc')->all();
$allActions = ArrayHelper::merge($actions, $withdraws);
ArrayHelper::multisort($allActions, ['time'], SORT_DESC);
$user = $model;
...
if ($matrix && ($matrix->children != 0 || $matrix->slots != 0)) {
    $canDeActive = false;
}
if ($user->parent_id){
    $canDeActive = false;
...
<div class="user-view">
    ...
    <p>
        <?
        if(User::isAccess('superadmin')) {
            echo Html::a('Конвертировать CV в PV', ['/deposit/convert', 'username' => $model->username], ['class' => 'btn btn-primary']);
    ...
    <p>
        <?
        if(User::isAccess('superadmin') && $canDeActive) {
            echo Html::a('Откат активации', ['/users/deactiv', 'userId' => $model->id], ['class' => 'btn btn-danger']);
        }
        ?>
    </p>
    <p>
    ...
    <?= DetailView::widget([
        ...
        'attributes' => [
            ...
            [
                ...
                'label' => 'Статистика баланса',
                'format' => 'raw',
                'value' => function($data){
                    return  Html::a('Подробнее ', ['users/balance-statistic', 'id' => $data['id']], ['class' => 'profile-link']);
                },
            ],
            [
                ...
                'format' => 'raw',
                'value' => function($data){
                    $verification = \common\models\Verifications::find()->where(['user_id' => $data['id']])->one();
                    if($verification){
                        return  Html::a('Просмотр ', ['verifications/view', 'id' => $verification['id']], ['class' => 'profile-link']);
                    }else{
                        return 'Не найдено или не верифицирован';
                    }
    ...
    <ul class="nav nav-tabs">
        ...
        <li class="<?if($tab == "panel8"){echo "active";}?>"><a href="/users/view?id=<?=$user['id']?>&tab=panel8">История изменений почты</a></li>
    </ul>
                ...
                <div class="tab-content">
                    <div id="panel100" class="tab-pane fade in active">
                        ...
                        <table class="table mt-5">
                            ...
                            <?
                            $i = 0;
                            foreach ($allActions as $withdraw) {
                                $i++;
                                ?>
                                <tr>
                                    <th scope="row"><?=$i?></th>
                                    ...
                                    <td style="text-align: right">
                                        <?
                                        if (array_key_exists('type', $withdraw)){
                                    ...
                                    <td>
                                        <?
                                        if (array_key_exists('title', $withdraw)) {
                                            echo $withdraw['title'];
                                            if ($withdraw['type'] == 3) {
                                            ...
                                            if($withdraw['system_id'] == 1){
                                                echo "Вывод Advcash (".$withdraw['account'].")";
                                            }elseif($withdraw['system_id'] == 2){
                                                echo "Вывод Perfect Money (".$withdraw['account'].")";
                                            }elseif($withdraw['system_id'] == 3){
                                                echo "Вывод Payeer (".$withdraw['account'].")";
                                            }
                                        }
                                        ?>
                                    </td>
                        ...
                        <table class="table table-striped table-bordered">
                            ...
                            foreach ($children as $child) {
                                ...
                                <tr data-key="36">
                                    ...
                                    <td>
                                        <?=date('d.m.Y H:i', $cours['time_personal']);?>
                                    </td>
                                </tr>
                            <?}?>
                            </tbody>
            ...
            <div id="panel8" class="tab-pane fade in active">
                ...
                <table class="table">
                    ...
                    <tr>
                        ...
                        <th class="col">Старая почта</th>
                        <th class="col">Новая почта</th>
                        <th class="col">Время смены</th>
                    </tr>
                    </thead>
                    <? $users = UserEmails::find()->where(['user_id'=>$user['id']])->all()?>
                        <tbody>
                        <?foreach ($users as $user):?>
                            <tr>
                                ...
                                <td class="col"><?=$user->old_email?></td>
                                <td class="col"><?=$user->new_email?></td>
                                <td class="col"><?=$user->time?></td>
                            </tr>
                        <?endforeach;?>
                        </tbody>
                </table>
