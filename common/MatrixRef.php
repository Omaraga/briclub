...
class MatrixRef extends \yii\db\ActiveRecord
{
        ...
        if(!empty($parent_matrix)){
            ...
            if($parent_matrix['children'] >1){
                ...
                if($parent_matrix->children == 0){
                    ...
                    if($parent_matrix->slots >0 and $shoulder_main['time']>1625138609){
                        $m_price = MLevelsNew::findOne($matrix->platform_id)['price'];
                        if($level == 1){
                            $m_price = 100;
                        }
                        if($level == 1){
                            User::plusBalans($pm_user['id'],$m_price);
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            $action_bon->sum = $m_price;
                            $action_bon->user_id = $parent_matrix->user_id;
                            $action_bon->user2_id = $user['id'];
                            $action_bon->title = "Начислены бонусы за место на площадке ".$parent_matrix->platform_id." за пользователя ".$user['username']."[".$matrix['id']."]";
                            $action_bon->type = 7;
                            $action_bon->save();
                        }
                        elseif ($level == 3){
                            User::plusBalans($pm_user['id'],700);
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            $action_bon->sum = 700;
                            ...
                            if(!empty($pm_user['parent_id'])){
                                User::plusBalans($pm_user['parent_id'],200);
                                $action_bon = new Actions();
                                $action_bon->time = time();
                                $action_bon->status = 1;
                                $action_bon->sum = 200;
                                $action_bon->user_id = $pm_user['parent_id'];
                                $action_bon->user2_id = $user['id'];
                                $action_bon->title = "Начислены кураторские бонусы на площадке ".$parent_matrix->platform_id." за пользователя ".$user['username']."[".$matrix['id']."]";
                                $action_bon->type = 105;
                                $action_bon->save();
                            }


                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-2,false,1);
                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-2,false,1);
                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-2,false,1);
                        ...
                        elseif ($level == 4){
                            ...
                            $children = User::find()->where(['parent_id'=>$user['id']])->count();
                            if($children>1){
                                User::plusBalans($pm_user['id'],1500);
                            }else{
                                User::plusBalans($pm_user['id'],1500,null,true);
                            }
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            ...
                            if(!empty($pm_user['parent_id'])){
                                User::plusBalans($pm_user['parent_id'],400);
                                $action_bon = new Actions();
                                $action_bon->time = time();
                                $action_bon->status = 1;
                                $action_bon->sum = 400;
                            ...
                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-3,false,1);
                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-3,false,1);
                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-3,false,1);
                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-3,false,1);
                        ...
                        elseif ($level == 5){
                            $children = User::find()->where(['parent_id'=>$user['id']])->count();
                            if($children>1){
                                User::plusBalans($pm_user['id'],5000);
                            }else{
                                User::plusBalans($pm_user['id'],5000,null,true);
                            }
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            ...
                            if(!empty($pm_user['parent_id'])){
                                User::plusBalans($pm_user['parent_id'],600);
                                $action_bon = new Actions();
                                $action_bon->time = time();
                                $action_bon->status = 1;
                                $action_bon->sum = 600;
                            ...
                            if($parent_matrix['id'] == 7490){

                            }else{
                                self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-4,false,1);
                                self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-4,false,1);
                                self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-4,false,1);
                        ...
                        elseif ($level == 6){
                            $children = User::find()->where(['parent_id'=>$user['id']])->count();
                            if($children>1){
                                User::plusBalans($pm_user['id'],13000);
                            }else{
                                User::plusBalans($pm_user['id'],13000,null,true);
                            }
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            $action_bon->sum = 13000;
                            $action_bon->user_id = $parent_matrix->user_id;
                            $action_bon->user2_id = $user['id'];
                            $action_bon->title = "Начислены бонусы за место на площадке ".$parent_matrix->platform_id." за пользователя ".$user['username']."[".$matrix['id']."]";
                            $action_bon->type = 7;
                            ...
                            if(!empty($pm_user['parent_id'])){
                                User::plusBalans($pm_user['parent_id'],3000);
                                $action_bon = new Actions();
                                $action_bon->time = time();
                                $action_bon->status = 1;
                                $action_bon->sum = 3000;
                            ...
                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-5,false,1);
                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-5,false,1);
                            self::plusToRefMatrix($parent_matrix['user_id'],$parent_matrix->platform_id-5,false,1);
            ...
            if(!empty($big_parent_matrix)){
                ...
                if($big_parent_matrix->slots == 1){
                    if($big_parent_matrix->children==2){
                        $m_price = MLevelsNew::findOne($matrix->platform_id)['price'];
                        if($level == 1){
                            $m_price = 100;
                        }
                        if($level == 1){
                            User::plusBalans($bp_user['id'],$m_price);
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            $action_bon->sum = $m_price;
                        ...
                        elseif ($level == 3){
                            User::plusBalans($bp_user['id'],700);
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            $action_bon->sum = 700;
                            ...
                            if(!empty($bp_user['parent_id'])){
                                User::plusBalans($bp_user['parent_id'],200);
                                $action_bon = new Actions();
                                $action_bon->time = time();
                                $action_bon->status = 1;
                                $action_bon->sum = 200;
                                $action_bon->user_id = $bp_user['parent_id'];
                                $action_bon->user2_id = $user['id'];
                                $action_bon->title = "Начислены кураторские бонусы на площадке ".$big_parent_matrix->platform_id." за пользователя ".$user['username']."[".$matrix['id']."]";
                                $action_bon->type = 105;
                                $action_bon->save();
                            }


                            self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-2,false,1);
                            self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-2,false,1);
                            self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-2,false,1);
                        ...
                        elseif ($level == 4){
                            ...
                            $children = User::find()->where(['parent_id'=>$user['id']])->count();
                            if($children>1){
                                User::plusBalans($bp_user['id'],1500);
                            }else{
                                User::plusBalans($bp_user['id'],1500,null,true);
                            }
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            ...
                            if(!empty($bp_user['parent_id'])){
                                User::plusBalans($bp_user['parent_id'],400);
                                $action_bon = new Actions();
                                $action_bon->time = time();
                                $action_bon->status = 1;
                                $action_bon->sum = 400;
                            ...
                            self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-3,false,1);
                            self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-3,false,1);
                            self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-3,false,1);
                            self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-3,false,1);
                        ...
                        elseif ($level == 5){
                            $children = User::find()->where(['parent_id'=>$user['id']])->count();
                            if($children>1){
                                User::plusBalans($bp_user['id'],5000);
                            }else{
                                User::plusBalans($bp_user['id'],5000,null,true);
                            }
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            ...
                            if(!empty($bp_user['parent_id'])){
                                User::plusBalans($bp_user['parent_id'],600);
                                $action_bon = new Actions();
                                $action_bon->time = time();
                                $action_bon->status = 1;
                                $action_bon->sum = 600;
                            ...
                            if($big_parent_matrix['id'] == 7490){

                            }else{
                                self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-4,false,1);
                                self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-4,false,1);
                                self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-4,false,1);
                        ...
                        elseif ($level == 6){
                            $children = User::find()->where(['parent_id'=>$user['id']])->count();
                            if($children>1){
                                User::plusBalans($bp_user['id'],13000);
                            }else{
                                User::plusBalans($bp_user['id'],13000,null,true);
                            }
                            $action_bon = new Actions();
                            $action_bon->time = time();
                            $action_bon->status = 1;
                            ...
                            if(!empty($bp_user['parent_id'])){
                                User::plusBalans($bp_user['parent_id'],3000);
                                $action_bon = new Actions();
                                $action_bon->time = time();
                                $action_bon->status = 1;
                                $action_bon->sum = 3000;
                            ...
                            if($big_parent_matrix['id'] == 7490){
                                ...
                                self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-5,false,1);
                                self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-5,false,1);
                                self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id-5,false,1);
                ...
                if($big_parent_matrix->slots == 3){
                    ...
                    if ($level == 6){
                        User::plusBalans($bp_user['id'],$m_price/2);
                        $action_bon = new Actions();
                        $action_bon->time = time();
                        $action_bon->status = 1;
                        $action_bon->sum = $m_price/2;
                        $action_bon->user_id = $big_parent_matrix->user_id;
                        $action_bon->user2_id = $user['id'];
                        $action_bon->title = "Начислены бонусы за место на площадке ".$big_parent_matrix->platform_id." за пользователя ".$user['username']."[".$matrix['id']."]";
                        $action_bon->type = 7;
                ...
                if($big_parent_matrix->slots == 4){
                    ...
                    if ($level == 6){
                        ...
                        if($big_parent_matrix->reinvest == 1){
                            $clon_num = null;
                            $clon = MatClons::find()->where(['mat_id'=>$big_parent_matrix['id']])->one();
                            if(empty($clon)){
                                $clon_num = $clon['num'];
                            }
                            self::plusToRefMatrix($big_parent_matrix['user_id'],$big_parent_matrix->platform_id+1,false,1,false,$clon_num);
                        }else{
    ...
    public static function plusToRefMatrix2($user_id,$level=1,$new=false,$reinvest=0,$buy=false,$clon_num=null,$without=null,$parmat=null){
        ...
        if($reinvest == 1){
            $action->type = 11;
            $action->title = "У вас новый клон [".($matrix->id)."] на уровене ".($matrix->platform_id);
        }
        ...
        $parent_matrix = MatrixRef::findOne($parmat);
...
//                            User::plusBalans($bp_user['parent_id'],200);
                        ...
                        if($bp_user['id'] == 12750 or $bp_user['id'] == 12751 or $bp_user['id'] == 12752){
