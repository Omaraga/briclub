    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index','block','unblock','withdrawcancel', 'get-course','verification','favorites','settings','courses','structure','actions','withdraw','activ','transfer','withdraws','perfect','bonus','bonus-global','refs','refs-global','matrix','children','getnewplatform','getnewplatformstart','tickets','documents','global','start', 'promo', 'pay-bill', 'user-bills', 'view-document','statistic', 'theme', 'delete-avatar', 'upload-avatar', 'convert', 'insurance','insurance-change-status','upload-file','excel'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
...
//                }
    ...
    public function actionExcel($level = 2){
        set_time_limit(0);
        //ini_set('memory_limit', '-1');
        $user = Yii::$app->user->identity;
        /* @var $matrixRef MatrixRef*/
        $children = ArrayHelper::getColumn(Referals::find()->select('user_id')->where(['parent_id' => $user->id])->all(),'user_id');
        $matrixRef = MatrixRef::find()->where(['user_id'=> $user->id, 'platform_id' => $level])->orderBy('id ASC')->one();

        $queue = [];

        array_push($queue, $matrixRef);
        $heightLeft = 0;
        $heightRight = 0;
        while(sizeof($queue) > 0)
        {
            ...
            echo "<br>";
            if($node['children'] > 0){
                $left = MatrixRef::find()->where(['id' => $node['shoulder1']])->select('id, shoulder1,shoulder2, user_id, children')->asArray()->one();
                $right = MatrixRef::find()->where(['id' => $node['shoulder2']])->select('id, shoulder1,shoulder2, user_id, children')->asArray()->one();;
                if(!empty($left)) {
                    array_push($queue, $left);
                    $heightLeft++;
                    //echo "LEFT: ".($left['id'])."<br>";
                }
                if(!empty($right) ) {
                    array_push($queue, $right);
                    $heightRight++;
                    //echo "RIGHT: ".($right['id'])."<br>";
                }
            }
            echo sizeof($queue).'<br>';
        }

        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        for($i=1; i<= max($heightLeft, $heightRight); $i++){
            for($j=1; $j<=$i; $j++){
