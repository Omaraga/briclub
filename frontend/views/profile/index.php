<main class="d-flex">
    <div class="main-content" id="main">

        <div class="main-block">

            <div class="main-list txt-5F7">
                <div class="list-item">
                    <h6>Резидент</h6>
                    <div class="list-item_line"></div>
                </div>
                <div class="list-item">
                    <h6>Компаньон</h6>
                    <div class="list-item_line"></div>
                </div>
                <div class="list-item">
                    <h6>Консул</h6>
                    <div class="list-item_line"></div>
                </div>
                <div class="list-item">
                    <h6>Кастодиан</h6>
                    <div class="list-item_line"></div>
                </div>
                <div class="list-item">
                    <h6>Меценат</h6>
                    <div class="list-item_line"></div>
                </div>
            </div>

            <div class="scroll">

                <div class="row mb-4">
                    <div class="col mb-3">
                        <div class="cards mx-auto">
                            <h6 class="mb-3">Баланс</h6>
                            <div class="center-line mb-3">
                                <div class="block-CV center">
                                    <h4 class="w7">CV <span class="ms-2">5000</span></h4>
                                </div>
                                <div class="block-PV center ms-3">
                                    <h4 class="w7">PV</h4>
                                </div>
                            </div>
                            <a href="#" class="fon-gray-800 py-2 px-4 text-white" style="border-radius: 4px;">Мои балансы</a>
                        </div>
                    </div>

                    <div class="col">
                        <div class="cards mx-auto">
                            <h6 class="mb-3">Доход с <span class="w7">Матрицы</span></h6>

                            <div class="fon-main between p-3 item-card">
                                <img src="img/main/cards-img.svg" alt="">
                                <div class="rows">
                                    <p class="txt-mini">За партнера</p>
                                    <h4 class="w5">pv 100</h4>
                                </div>
                                <p class="txt-mini txt-green-100">+ 100%
                                <p>
                            </div>

                            <div class="fon-main between p-3 mt-3 item-card mb-4">
                                <img src="img/main/cards-img.svg" alt="">
                                <div class="rows">
                                    <p class="txt-mini">За партнера</p>
                                    <h4 class="w5">pv 100</h4>
                                </div>
                                <p class="txt-mini txt-green-100">+ 100%
                                <p>
                            </div>
                            <a href="#" class="fon-btn-blue-100 py-2 px-4 text-white" style="border-radius: 4px;">Пригласить в клуб</a>
                        </div>
                    </div>
                </div>

                <div class="row">
                    <div class="col">
                        <div class="cards mx-auto">
                            <h5 class="mb-3">Мои акции</h5>
                            <div class="banner item-card">
                                <div class="banner-header between">
                                    <div class="d-flex">
                                        <img src="img/main/banner-bri.svg" alt="">
                                        <h5 class="w7 center ms-2">BRI</h5>
                                    </div>
                                    <p class="text-mini">1 BRI = 2$</p>
                                </div>
                                <h4 class="w7">12 000 BRI</h4>
                                <h5 class="w5 txt-AAA">24 000 $</h5>
                            </div>
                        </div>
                    </div>

                    <div class="col margin-top-38">
                        <div class="cards mx-auto">
                            <div class="banner banner-2 item-card">
                                <div class="banner-header between">
                                    <div class="d-flex">
                                        <img src="img/main/banner-grc.svg" alt="">
                                        <h5 class="w7 center ms-2">GRC</h5>
                                    </div>
                                    <p class="text-mini">1 BRI = 2$</p>
                                </div>
                                <h4 class="w7">12 000 BRI</h4>
                                <h5 class="w5 txt-AAA">24 000 $</h5>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="row">
                    <div class="col">
                        <div class="cards mx-auto">
                            <h6 class="w5 activity-title">Мероприятия</h6>
                            <div class="activity fon-main one">
                                <div class="activity-block">
                                    <div class="rows mb-3">
                                        <h4 class="w7">22</h4>
                                        <h6>сентября</h6>
                                    </div>
                                    <div class="rows">
                                        <h6 class="txt-green">Бизнес завтрак</h6>
                                        <h5 class="w7">Инвестируйте в свое будущее</h5>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col mb-3" style="margin-top: 78px;">
                        <a href="">
                            <div class="cards mx-auto text-white">
                                <div class="activity fon-main two">
                                    <div class="activity-block">
                                        <div class="rows mb-3">
                                            <h4 class="w7">22</h4>
                                            <h6>сентября</h6>
                                        </div>
                                        <div class="rows">
                                            <h6 class="txt-green">Бизнес завтрак</h6>
                                            <h5 class="w7">Инвестируйте в свое будущее</h5>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                </div>

            </div>
        </div>
    </div>
    <aside class="fon-main px-3" id="aside">
        <div>
            <h6 class="mb-2">Доступно мне</h6>

            <div class="cards fon-gray-300 between">
                <div class="d-flex align-items-center">
                    <div class="circle fon-gray-200 me-2 mb-1"></div>
                    <h6>Онлайн мастер класс</h6>
                </div>
                <div class="date center rows">
                    <h6>22.09</h6>
                    <h6>18:00</h6>
                    <div class="circle center">1</div>
                </div>
            </div>

            <div class="cards fon-gray-300 between">
                <div class="d-flex align-items-center">
                    <div class="circle fon-green-200 me-2 mb-1"></div>
                    <h6>Онлайн мастер класс</h6>
                </div>
                <div class="date center rows">
                    <h6>22.09</h6>
                    <h6>18:00</h6>
                    <div class="circle center">1</div>
                </div>
            </div>

            <div class="cards fon-gray-300 between">
                <div class="d-flex align-items-center">
                    <div class="circle fon-green-200 me-2 mb-1"></div>
                    <h6>Онлайн мастер класс</h6>
                </div>
                <div class="date center rows">
                    <h6>22.09</h6>
                    <h6>18:00</h6>
                    <div class="circle center">1</div>
                </div>
            </div>

            <div class="cards fon-gray-300 between">
                <div class="d-flex align-items-center">
                    <div class="circle fon-green-200 me-2 mb-1"></div>
                    <h6>Онлайн мастер класс</h6>
                </div>
                <div class="date center rows">
                    <h6>22.09</h6>
                    <h6>18:00</h6>
                    <div class="circle center">1</div>
                </div>
            </div>
        </div>

        <div class="mt-5 margin-bot-32">

            <h6 class="mb-3">Мои подарки</h6>

            <div class="cards fon-aside-card-100 between">
                <div>
                    <h6>Страховка Каско</h6>
                </div>
                <div class="date center">
                    <p>1</p>
                </div>
            </div>

            <div class="cards fon-green-400 between">
                <div>
                    <h6>Страховка Каско</h6>
                </div>
                <div class="date center">
                    <p>2</p>
                </div>
            </div>
        </div>

        <div>

            <h6 class="mb-3">Календарь событий</h6>


        </div>

    </aside>
</main>